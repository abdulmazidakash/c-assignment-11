...
function FamousDiscoverers() {
  ...
  return (
    <div className="bg-gray-100 dark:bg-gray-900 shadow-lg px-6 py-12 md:py-16 lg:py-20 my-8 rounded-2xl container mx-auto">
       {/* Page Title */}
       <h2 
      ...
      >
      <FaAward/> Famous Discoverers
      </h2>

      <p 
        className="text-lg text-center text-gray-600 dark:text-gray-300 mt-4 max-w-3xl mx-auto mb-10"
      >
        Remarkable Discoveries That Shaped Human History and Civilization.
      </p>
      ...
      <Marquee gradient={false} speed={50} pauseOnHover>
        <div className="flex gap-8">
          {discoverers.map((person, index) => (
            <div
              key={index}
              className="flex flex-col items-center text-center p-6 border rounded-xl bg-white dark:bg-gray-800 shadow-md hover:scale-105 transition-transform duration-300"
            >
              <img
                src={person.image}
                alt={person.name}
              ...
              />
              <h3 className="text-xl font-semibold text-rose-900 dark:text-rose-400">{person.name}</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">{person.achievement}</p>
            </div>
          ))}
        </div>
      </Marquee>
